...
apt install -y unzip libssl-dev libtool-bin dpkg-dev libbz2-dev bzip2 gcc g++ make libpcre3-dev
...
export PATH=\$PATH:/usr/xray:/home/service/nginx/sbin
EOF
cat >>/etc/profile<<EOF
#PATH GO
...
mv go /usr/local
# 安装xray
wget https://github.com/XTLS/Xray-core/releases/download/v1.2.3/Xray-linux-64.zip
rm -rf /usr/xray
...
AmbientCapabilities=CAP_NET_ADMIN CAP_NET_BIND_SERVICE
NoNewPrivileges=true
ExecStart=/usr/v2ray/v2ray -config /etc/xray/config.json
Restart=on-failure
RestartPreventExitStatus=23
LimitNPROC=500
LimitNOFILE=1000000
...
WantedBy=multi-user.target
EOF
# 创建Xray / v2ray VLESS-TLS-Nginx服务端配置
cat <<EOF >/etc/xray/config.json
{
  ...
  "inbounds": [{
    ...
    "settings": {
      "clients": [{
        "id": "54b358d5-7b4d-4451-863c-575bc66e1961",
        "level": 0,
...
wget https://github.com/hsgflt88/Newifi-D2/raw/main/index3.zip
unzip -o -d /home/service/xray index3.zip
...
tar -xvf node-v15.6.0-linux-x64.tar.xz
mv node-v15.6.0-linux-x64 /usr/nodejs
source /etc/profile
sleep 3
